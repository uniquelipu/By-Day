...
import com.common.calendar.CalendarConfig;
...
public class AlertReceiver extends WakefulBroadcastReceiver {
        ...
        synchronized (mStartingServiceSync) {
            if (mStartingService == null) {
                ...
                mStartingService = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                        "ByDay:StartingAlertService");
                mStartingService.setReferenceCounted(false);
            }
            mStartingService.acquire();
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                AlarmSchedulingService.enqueueWork(context,
                        AlarmSchedulingService.class, AlarmSchedulingService.JOB_ID, intent);
            } else {
    ...
    private static PendingIntent createClickEventIntent(Context context, long eventId,
                                                        String title, String summaryText, long startMillis, long endMillis, int notificationId, String accountType) {
        return createDismissAlarmsIntent(context, eventId, title, summaryText, startMillis, endMillis, notificationId,
                "com.android.calendar.CLICK", true, accountType);
    }

    private static PendingIntent createDeleteEventIntent(Context context, long eventId,
                                                         String title, String summaryText, long startMillis, long endMillis, int notificationId,
                                                         String accountType) {
        return createDismissAlarmsIntent(context, eventId, title, summaryText, startMillis, endMillis, notificationId,
                "com.android.calendar.DELETE", false, accountType);
    }

    private static PendingIntent createDismissAlarmsIntent(Context context, long eventId,
                                                           String title, String summaryText, long startMillis, long endMillis, int notificationId, String action,
                                                           boolean showEvent, String accountType) {
        Intent intent = new Intent();
        intent.setClass(context, DismissAlarmsService.class);
        intent.putExtra(AlertUtils.EVENT_ID_KEY, eventId);
        intent.putExtra(AlertUtils.EVENT_TITLE_KEY, title);
        intent.putExtra(AlertUtils.EVENT_SUMMARY_KEY, summaryText);
        ...
        intent.putExtra(AlertUtils.ACCOUNT_TYPE_KEY, accountType);
        ...
        // event OWNER_ID + startTime should be unique).  This also must be unique from
        // the deleteBySyncId event (which also uses DismissAlarmsService).
    ...
    private static PendingIntent createSnoozeIntent(Context context, long eventId, String title, String summary,
                                                    long startMillis, long endMillis, int notificationId, String accountType) {
        Intent intent = new Intent();
        intent.putExtra(AlertUtils.EVENT_ID_KEY, eventId);
        intent.putExtra(AlertUtils.EVENT_TITLE_KEY, title);
        intent.putExtra(AlertUtils.EVENT_SUMMARY_KEY, summary);
        intent.putExtra(AlertUtils.EVENT_START_KEY, startMillis);
        intent.putExtra(AlertUtils.EVENT_END_KEY, endMillis);
        intent.putExtra(AlertUtils.NOTIFICATION_ID_KEY, notificationId);
        intent.putExtra(AlertUtils.ACCOUNT_TYPE_KEY, accountType);
    ...
    private static PendingIntent createAlertActivityIntent(Context context, String accountType) {
        Intent clickIntent = new Intent();
        clickIntent.setClass(context, AlertActivity.class);
        clickIntent.putExtra(AlertActivity.KEY_ACCOUNT_TYPE, accountType);
    ...
    private static PendingIntent createAlertActivityIntent(Context context) {
        Intent clickIntent = new Intent();
        clickIntent.setClass(context, AlertActivity.class);
        clickIntent.putExtra(AlertActivity.KEY_ACCOUNT_TYPE, CalendarConfig.GOOGLE_CALENDAR_ACCOUNT_TYPE);
        clickIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        return PendingIntent.getActivity(context, 0, clickIntent,
                PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_UPDATE_CURRENT);
    }
    ...
    public static NotificationWrapper makeBasicNotification(Context context, String title,
                                                            ...
                                                            int notificationId, boolean doPopup, int priority, String accountType) {
        Notification n = buildBasicNotification(new Notification.Builder(context),
                ...
                doPopup, priority, accountType, false);
        return new NotificationWrapper(n, notificationId, eventId, startMillis, endMillis, doPopup);
    }

    public static NotificationType getNotificationType(String title) {
        if (title.toLowerCase().contains(NotificationType.BIRTHDAY.name)) {
            return NotificationType.BIRTHDAY;
        } else if (title.toLowerCase().contains(NotificationType.ANNIVERSARY.name)) {
            return NotificationType.ANNIVERSARY;
        } else if (title.toLowerCase().contains(NotificationType.VALENTINE.name)) {
            return NotificationType.VALENTINE;
        } else if (title.toLowerCase().contains(NotificationType.MITZVAH.name)) {
            return NotificationType.MITZVAH;
        } else if (title.toLowerCase().contains(NotificationType.BAPTISM.name)) {
            return NotificationType.BAPTISM;
        } else if (title.toLowerCase().contains(NotificationType.GRADUAT.name)) {
            return NotificationType.GRADUAT;
        } else if (title.toLowerCase().contains(NotificationType.SHOWER.name)) {
            return NotificationType.SHOWER;
        } else if (title.toLowerCase().contains(NotificationType.FUNERAL.name)) {
            return NotificationType.FUNERAL;
        } else if (title.toLowerCase().contains(NotificationType.FATHERS.name)) {
            return NotificationType.FATHERS;
        } else if (title.toLowerCase().contains(NotificationType.MOTHERS.name)) {
            return NotificationType.MOTHERS;
        } else {
            return NotificationType.OTHER;
        }
    }

    public static boolean isSpecialEvent(String title) {
        NotificationType type = getNotificationType(title);
    ...
    private static Notification buildBasicNotification(Notification.Builder notificationBuilder,
                                                       ...
                                                       long eventId, int notificationId, boolean doPopup, int priority, String accountType,
        ...
        PendingIntent clickIntent = createClickEventIntent(context, eventId, title, summaryText, startMillis,
                endMillis, notificationId, accountType);

        // Create a deleteBySyncId intent triggered by dismissing the notification.
        PendingIntent deleteIntent = createDeleteEventIntent(context, eventId, title, summaryText, startMillis,
                endMillis, notificationId, accountType);

        // Create the base notification.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            notificationBuilder.setColor(ContextCompat.getColor(context, R.color.blue_primary));
        }
        notificationBuilder.setContentTitle(title);
        notificationBuilder.setContentText(summaryText);
        notificationBuilder.setSmallIcon(R.drawable.stat_notify_calendar);
        setLargeIcon(notificationBuilder, context, title);
        ...
        if (addActionButtons) {
            ...
            snoozeIntent = createSnoozeIntent(context, eventId, title, summaryText, startMillis, endMillis,
                    notificationId, accountType);
        }
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            if (snoozeIntent != null && numActions < MAX_NOTIF_ACTIONS) {
                notificationBuilder.addAction(R.drawable.ic_snooze_white,
                        resources.getString(R.string.snooze_label), snoozeIntent);
                numActions++;
            }
            if (deleteIntent != null && numActions < MAX_NOTIF_ACTIONS) {
                notificationBuilder.addAction(R.drawable.ic_dismiss_white,
                        resources.getString(R.string.dismiss_label), deleteIntent);
                numActions++;
            }
    ...
    private static void setLargeIcon(Notification.Builder notificationBuilder, Context context, String title) {
        NotificationType notificationType = getNotificationType(title);
        notificationBuilder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), notificationType.imgResId));
    }
    ...
    static Intent createEmailIntent(Context context, long eventId, String body) {
        // TODO: Refactor to move query part into Util.createEmailAttendeeIntent, to
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (DELETE_ALL_ACTION.equals(intent.getAction())) {
            ...
            // fired), updateByRecipeId the notifications to express this change.
            ...
            if (eventId != -1) {
                ...
                if (geoIntent != null) {
                    ...
                    // No location was found, so updateByRecipeId all notifications.
            ...
            if (eventId != -1) {
                ...
                if (callIntent != null) {
                    ...
                    // No call location was found, so updateByRecipeId all notifications.
                    // Our alert service does not currently allow us to specify only one
                    // specific notification to refresh.
                    AlertService.updateAlertNotification(context);
                }
    ...
    public enum NotificationType {
        BIRTHDAY("birthday", R.drawable.ic_stat_holiday),
        ANNIVERSARY("anniv", R.drawable.ic_stat_holiday),
        VALENTINE("valentine", R.drawable.ic_stat_holiday),
        MITZVAH("mitzvah", R.drawable.ic_stat_holiday),
        BAPTISM("baptism", R.drawable.ic_stat_holiday),
        GRADUAT("graduat", R.drawable.ic_stat_holiday),
        SHOWER("shower", R.drawable.ic_stat_holiday),
        FUNERAL("funeral", R.drawable.ic_stat_holiday),
        FATHERS("father's", R.drawable.ic_stat_holiday),
        MOTHERS("mother's", R.drawable.ic_stat_holiday),

        OTHER("other", R.drawable.ic_stat_notification_normal);

        public final String name;
        public final int imgResId;

        NotificationType(String name, int imgResId) {
            this.name = name;
    ...
    public static class AlarmSchedulingService extends JobIntentService {
        ...
        protected void onHandleWork(Intent intent) {
            ...
            //startWakefulService(getContext, intent);
        }
